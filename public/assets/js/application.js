$(".counter").each(function() {
  var $this = $(this),
    ...
    isCur = $this.attr("data-currency"),
    currency = $this.attr("data-currency") === "1" ? "₱" : "",
    pc = $this.attr("data-percent") === "1" ? "%" : "";
  $({
    countNum: $this.text()
  }).animate(
    {
    ...
    },
    {
      ...
      step: function() {
        if (isCur === "1") {
          $this.text(currency + numeral(this.countNum).format("0,0.00") + pc);
        } else {
          $this.text(currency + numeral(this.countNum).format("0,0") + pc);
        }
      },
      complete: function() {
        if (isCur === "1") {
...
$(function() {
  ...
  $("body").on("click", "ul.acc-menu a", function() {
    ...
    if (
      !$("body").hasClass("collapse-leftbar") ||
