        ...
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Session Cancellation</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                    </button>
                  </div>
            ...
            <div class="modal-footer">
              <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
              <button type="button" id='updateSession' class="btn btn-warning" data-dismiss="modal">Update Session</button>
...
$.ajax({
    ...
    url: '{{ route("pt.session.change", ["pt" => $pt->id]) }}/' + $('#sessionId').val() + '/edit',
    type: 'POST',
    data: {
        price: $('#editSession #session-price').val(),
        client_id: $('#editSession #session-client').val(),
        sessions_type: $('#editSession #session-type').val(),
        notes: $('#editSession #session-notes').val(),
        total_sessions: $('#editSession #total-sessions').val(),
        session_number: $('#editSession #session-number').val(),
        payment_mode: $('#editSession #payment-mode').val(),
        date_purchased: $('#editSession #date-purchased').val(),
        expiry: $('#editSession #expiry').val(),
        sessions_left: $('#editSession #sessions-left').val(),
    },
    error: function (data) {
        alert("Error");
    },
...
d.push({
    ...
    durationEditable: false
            ...
            eventClick: function(calEvent, jsEvent, view) {
                let s = calEvent.details;
                $('#sessionId').val(calEvent.event_id);
                $('#editSession #session-client').val(s.client_id);
                $('#editSession #session-type').val(s.sessions_type);
                $('#editSession #session-price').val(s.price);
                $('#editSession #total-sessions').val(s.total_sessions);
                $('#editSession #session-number').val(s.session_number);
                $('#editSession #payment-mode').val(s.payment_mode);
                $('#editSession #date-purchased').val(s.date_purchased);
                $('#editSession #expiry').val(s.expiry);
                $('#editSession #sessions-left').val(s.sessions_left);
                $('#editSession #session-notes').val(s.notes);
